...
<li class='mt-2 '><code>s</code> consists of parentheses only <code class="text-md">'()[]{}'</code>.</li>`,
	...
	order: 4,
	starterCodePython: `def validParentheses(s):
    # Write your code here
    pass`,
...
tests = ["()", "()[]{}", "(]", "([)]"]
for s in tests:
    print(str(validParentheses(s)).lower())`,
...
int main() {
    string tests[] = {"()", "()[]{}", "(]", "([)]"};
    for (auto& s : tests) {
        cout << (validParentheses(s) ? "true" : "false") << endl;
    }
    return 0;
}`,
		java: `{{USER_CODE}}
...
class Main {
    public static void main(String[] args) {
        ...
        String[] tests = {"()", "()[]{}", "(]", "([)]"};
        for (String s : tests) {
            System.out.println(sol.validParentheses(s));
        }
    }
}`,
	},
